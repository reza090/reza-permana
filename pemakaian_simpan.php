...
if(isset($_POST['tambahbtn'])){
	if(!$_POST['kdbarangtxt'] || !$_POST['jmltxt']){
		...
		echo "<meta http-equiv='refresh' content='0;home.php?go=pemakaian_form'>";		
	}else {
		$sql = "INSERT INTO tbltransaksi_pemakaian_rinci(no_pemakaian,kode_barang,harga_perkilo,jumlah,sub_total) VALUES ('$_POST[nofakturtxt]','$_POST[kdbarangtxt]','$_POST[hrgjualtxt]','$_POST[jmltxt]','$_POST[subtotaltxt]')";
		...
		if ($proses) {			
			...
			echo "<meta http-equiv='refresh' content='0;home.php?go=pemakaian'>";
		}
	}
}else{
	$sql = "select * from tbltransaksi_pemakaian_rinci where no_pemakaian ='$_POST[nofakturtxt]'";
	$proses = mysql_query($sql);
	$record = mysql_num_rows($proses);
	if ($record>=1){
				
		$sql_total = "select sum(sub_total) as gtotal from tbltransaksi_pemakaian_rinci where no_pemakaian ='$_POST[nofakturtxt]'";
		...
		$sql1 = "INSERT INTO tbltransaksi_pemakaian(no_pemakaian,tanggal,shift,total) VALUES ('$_POST[nofakturtxt]','$tgl','','$record_total[gtotal]')";
